...
func OnGifAndAttachment(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if common.IsGuildOwner(&discordgo.Guild{ID: m.GuildID}, m.Author.ID) { return }
	if common.HasAdminRole(&discordgo.Guild{ID: m.GuildID}, m.Member) { return }
	for _, id := range append(common.PrivilegedRoleIDs, common.StaffRoleIDs...) {
		if len(m.Member.Roles) > 0 && slices.Contains(m.Member.Roles, id) { return }
	}
